...
(function ($) {
    ...
    var flashAgency = $.flashAgency = function (options) {
        var _id = createRandom();
        options = options || {};
        config = $.extend(config, options);

        var swfTarget = _getFlash();
        var url = options.url;
        ...
        if (!swfTarget || swfTarget.nodeType == 1 && swfTarget.nodeName.toLocaleLowerCase() != "object") {
        ...
        if (!_isConnect) {
            _timer = setTimeout(function(){
                _self.call(null, options)
            }, 200);
        }

        //保存回调集合
        if (!flashAgency.callbacks) {
            flashAgency.callbacks = {};
        }
